...
class EVDayTrip():
    ...
    def make_tripdist(self):
        file_path = r'C:\Users\kiuch\Documents\program\20240714_EnergyManagementSimulation\TripDistance.xlsx'


        df_TripDistance = pd.read_excel(file_path,skiprows=2)
    ...
    def make_triptime(self):
        file_path = r'C:\Users\kiuch\Documents\program\20240714_EnergyManagementSimulation\TripHour.xlsx'
        df_TripHour = pd.read_excel(file_path,skiprows=2)
    ...
    def make_tripcount(self,region_code=100):
        file_path = r'C:\Users\kiuch\Documents\program\20240714_EnergyManagementSimulation\TripCount.xlsx'
        df_TripCount = pd.read_excel(file_path,skiprows=2)
...
class EVTrip():
    ...
    def cal_TripRate(self,region_code=100):
        file_path = r'C:\Users\kiuch\Documents\program\20240714_EnergyManagementSimulation\TripRatio.xlsx'
        df_TripRate = pd.read_excel(file_path,skiprows=2)
...
class EVGroupTrip():
    def __init__(self,Region,
        ...
        self.Model_TripDistance = 8856.633333333333
...
class EVGroupCharge():
    def __init__(self,
                ...
                Generation,
                Is_opt_capacity,
                Default_solarcapacity = 1,
                ControlRatio=1,
                Is_BESScal=False,
                energy_price_factor=1.0,
                contract_price_factor=1.0
                ):
        ...
        self.energy_price_factor = energy_price_factor
        self.contract_price_factor = contract_price_factor
        ...
        self.constraints=[]
        self.energycost=cp.Variable((1,),name="energycost",pos=True)
        self.contractcost=cp.Variable((1,),name="contractcost",pos=True)
        self.Is_BESScal = Is_BESScal
        self.results = 0

        if self.Is_BESScal:
            self.BESS_BatteryCapacity=cp.Variable((1,),name="BattryCapacity",pos=True)
            self.constraints.append(self.BESS_BatteryCapacity>=0)
            self.BESS_ChargeCapacity = 6
            self.BESS_Charge = cp.Variable((self.n_range,),name="BESS_Charge",pos=True)
            self.BESS_Discharge = cp.Variable((self.n_range,),name="BESS_Discharge",pos=True)
            self.BESS_ChargeEfficiency = 0.9
            self.BESS_Energy = cp.Variable((self.n_range+1,),name="BESS_Energy",pos=True)
            
            self.constraints.append(self.BESS_Charge>=0)
            self.constraints.append(self.BESS_Discharge>=0)
            self.constraints.append(self.BESS_Charge<=self.BESS_ChargeCapacity)
            self.constraints.append(self.BESS_Discharge<=self.BESS_ChargeCapacity)
            self.constraints.append(self.BESS_Energy<=self.BESS_BatteryCapacity)
            self.constraints.append(self.BESS_Energy>=0)
            
            self.constraints.append(self.BESS_Energy[0]==self.BESS_BatteryCapacity*0.5)
            self.constraints.append(self.BESS_Energy[-1]==self.BESS_BatteryCapacity*0.5)
            self.constraints.append(self.BESS_Energy[1:]==self.BESS_Energy[:-1]+self.BESS_Charge*self.BESS_ChargeEfficiency-self.BESS_Discharge/self.BESS_ChargeEfficiency)
        else:
            self.BESS_BatteryCapacity = 0
            self.BESS_Charge = np.zeros(self.n_range)
            self.BESS_Discharge = np.zeros(self.n_range)
            self.BESS_Energy = np.zeros(self.n_range+1)
        ...
        for UserType in self.List_UserType:
            ...
            for Control in [True, False]:
                ...
                if Control==True:
                    self.opt_EVGroupChargehour[UserType][Control]   = cp.Variable((self.n_range,),name="Charge_"+UserType,pos=True)
                    self.opt_EVGroupDischargehour[UserType][Control]= cp.Variable((self.n_range,),name="Discharge_"+UserType,pos=True)
                    self.opt_EVGroupEnergy[UserType][Control]       = cp.Variable((self.n_range+1,),name="Energy_"+UserType,pos=True)
                    ...
                    self.nonopt_EVGroupEnergy[UserType][Control] = np.zeros(self.n_range+1)
                ...
                self.Output_EVGroupEnergy[UserType][Control]  = np.empty(self.n_range+1)
        ...
        self.solve_linearproblem(self.objective,self.constraints)
    ...
    def solve_linearproblem(self, objective, constraints):
        if (self.Num_EV == 0) and (not self.Is_opt_capacity):
            self.results = 0
            self.status = "skipped"
            return

        self.model = cp.Problem(cp.Minimize(objective), constraints)
        try:
            self.results = self.model.solve(verbose=False) 
    ...
    def set_EVGroupCharge(self):
        ...
        self.objective = cp.Variable((1,),name="objective")
        ...
        self.Demand_cur = self.Demand- self.solar_capacity*self.Generation +self.BESS_Charge-self.BESS_Discharge
        for UserType in self.List_UserType:
            
            #for Control in [True,False]:
            #Control = bool(np.random.binomial(1,self.ControlRatio))
            Control = (self.ControlRatio >= 0.5)  # 0ならFalse, 1ならTrue
            ...
            if Control==True:
                ...
                self.constraints.append(self.opt_EVGroupEnergy[UserType][Control][1:] == self.opt_EVGroupEnergy[UserType][Control][:-1] + self.opt_EVGroupChargehour[UserType][Control]*self.ChargeEfficiency - self.opt_EVGroupDischargehour[UserType][Control]*(1/self.ChargeEfficiency)- trip/self.TripEfficiency)
        ...
        default_energycost=self.cal_energycost(sum(self.Demand),None)
        default_contractcost=self.cal_contractcost(max(self.Demand))
        initialcost,InitialCost_life,benefit =  self.cal_cost(self.energycost,self.contractcost,self.solar_capacity,Num_V2H,Num_NC,self.BESS_BatteryCapacity,default_energycost,default_contractcost)
        if (self.Num_EV==0)&(not self.Is_opt_capacity):
            self.cost=default_energycost+default_contractcost
            self.energycost=default_energycost
            self.contractcost=default_contractcost
        else:
            self.cost=self.energycost+self.contractcost
            #self.constraints.append(self.objective == initialcost*cp.inv_pos(benefit))
            self.constraints.append(self.objective == InitialCost_life-benefit)
        

    def cal_energycost(self, ComsumtionEnergy, Trip):
        Price_ComsumptionEnergy = 16 * self.energy_price_factor
        energycost = ComsumtionEnergy * Price_ComsumptionEnergy / (10**4)
        return energycost

    
    def cal_contractcost(self, ContractPower):
        Price_ContaractPower = 1684 * self.contract_price_factor
        contractcost = ContractPower * Price_ContaractPower / (10**4) * 12
        return contractcost

    def cal_cost(self,energycost,contractcost,SolarCapacity,Num_V2H,Num_NC,BESS_BatteryCapacity,default_energycost,default_contractcost):
        annual_benefit = default_energycost+default_contractcost-energycost-contractcost
        ...
        #Cost = energycost+contractcost+SolarCapacity * (Price_ini_Solarpanel/lifetime_Solarpanel+Price_om_Solarpanel)+Num_V2H*(Price_init_V2H/lifetime_V2H+Price_om_V2H)+Num_NC*(Price_init_NC/lifetime_NC+Price_om_NC)+BESS_BatteryCapacity*(Price_init_BESS/lifetime_BESS+Price_om_BESS)
        InitialCost = SolarCapacity * Price_ini_Solarpanel+Num_V2H*Price_init_V2H+Num_NC*Price_init_NC+BESS_BatteryCapacity*Price_init_BESS
        InitialCost_life=SolarCapacity * (Price_ini_Solarpanel/lifetime_Solarpanel+Price_om_Solarpanel)+Num_V2H*(Price_init_V2H/lifetime_V2H+Price_om_V2H)+Num_NC*(Price_init_NC/lifetime_NC+Price_om_NC)+BESS_BatteryCapacity*(Price_init_BESS/lifetime_BESS+Price_om_BESS)
        Benefit = annual_benefit
        
        return(InitialCost,InitialCost_life,Benefit)
    ...
    def cal_EVGroupCharge(self):
        ...
        if self.Is_BESScal:
            self.BESS_BatteryCapacity = self.BESS_BatteryCapacity.value[0]
            self.BESS_Charge = self.BESS_Charge.value
            self.BESS_Discharge = self.BESS_Discharge.value
            self.BESS_Energy = self.BESS_Energy.value
        ...
        self.Dict_summary["BESS_BatteryCapacity"]=self.BESS_BatteryCapacity
        if (self.Num_EV==0)&(not self.Is_opt_capacity):
            self.Dict_summary["BuyPowerEnergy"]=sum(self.Demand)
            self.Dict_summary["ContractPower"]=max(self.Demand)
            self.Dict_summary["Cost"]=self.cost
            self.Dict_summary["EnergyCost"]=self.energycost
            self.Dict_summary["ContractCost"]=self.contractcost
        else:
        ...
        self.Dict_summary["Is_opt_solarcapacity"]=self.Is_opt_capacity
        self.Dict_summary["Is_opt_BESS"]=self.Is_BESScal
